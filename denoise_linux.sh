...
DEEPFILTERNET_PATH="$SCRIPT_DIR/bin/deep-filter-0.5.6-x86_64-unknown-linux-musl"
...
for FILE in "$OUTPUT_DIR"/*.wav; do
    ...
    ffmpeg -i "$FILE" -af "highpass=f=100, equalizer=f=250:t=q:w=1:g=-3, equalizer=f=1000:t=q:w=1:g=1.5, acompressor=threshold=-20dB:ratio=3:attack=5:release=50, deesser=i=0.5, loudnorm=I=-16:TP=-2.5:LRA=11, volume=0.9, alimiter=limit=0.9" "$OUTPUT_FILE"
    if [ $? -ne 0 ]; then
